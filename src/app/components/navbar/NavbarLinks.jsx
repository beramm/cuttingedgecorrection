    ...
    <>
      <ul className="hidden md:flex items-center justify-center gap-x-8">
        {navbarMenu.map((eachMenu, index) => {
          return (
            <li key={index} className="py-4 flex gap-x-1 items-center justify-center cursor-pointer group text-xl">
              {/* {eachMenu.name === 'Subscription Plans' ? <Link href={"/subscription-plans"}>{eachMenu.name}</Link> : eachMenu.name} */}
              {eachMenu.link ? (
                <Link href={eachMenu.link}>{eachMenu.name}</Link>
              ) : (
                eachMenu.name
              )}
              ...
              {eachMenu.subMenu &&
                ...
                <div className="text-base absolute w-full max-w-xl top-48 left-1/2 transform -translate-x-1/2 -translate-y-28 z-50 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-300">
            ...
            return (
              <li key={index} className="mb-8 w-full">
                <div className="flex items-center justify-between" onClick={() => openSubmenu !== eachMenu.name ? setOpenSubmenu(eachMenu.name) : setOpenSubmenu("")}>
                  ...
                  {eachMenu.link ? (
                    <Link href={eachMenu.link} onClick={handleOpenMobileNav}>
                      {eachMenu.name}
                    </Link>
                  ) : (
                    eachMenu.name
